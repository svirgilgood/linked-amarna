                ...
                for key, items in item_dict.items():
                    if key == "UF":
                        ...
                        try:
                            ...
                            CUR.execute(
                                query.format(table_name, column, items, name_id, name)
                            )
                        except KeyError:
                            pass
                num += 1
...
def main():
    ...
    arg_parser.add_argument(
        "-u",
        "--use",
        help="specify a WPACV yaml file to fill in use and related terms. Must be run after the database is built",
    )
